...
<div id="container">
	...
	<%	 
     ...
     try{
	   ...
	   //select * from noticeboard n full outer join qnaboard q on n.writerB = 'mac' and q.writerQ = 'mac'
	   //정렬 조건 : 히트 | 작성일
	   if(request.getParameter("col") != null && request.getParameter("word") != null){
		  ...
		  if(subject_search.equals("name")){ 
			...
			if(request.getParameter("search") != null && request.getParameter("search").equals("date")){
				sql="select * from noticeboard n inner join qnaboard q on n.writerB = '"+(String)session.getAttribute("name")+"' and q.writerQ = '"+(String)session.getAttribute("name")+"' order by 9 desc";
				
			}else if(request.getParameter("search") != null && request.getParameter("search").equals("hit")){
				sql="select * from noticeboard n inner join qnaboard q on n.writerB = '"+(String)session.getAttribute("name")+"' and q.writerQ = '"+(String)session.getAttribute("name")+"' order by 7 desc";
				
			}else if(request.getParameter("search") != null && request.getParameter("search").equals("notice")){
				sql="select * from noticeboard where writerB = '"+(String)session.getAttribute("name")+"'";
				
			}else if(request.getParameter("search") != null && request.getParameter("search").equals("qna")){
				sql="select * from qnaboard where writerQ = '"+(String)session.getAttribute("name")+"'";
				
			}else{
				sql="select * from noticeboard n inner join qnaboard q on n.writerB = '"+(String)session.getAttribute("name")+"' and q.writerQ = '"+(String)session.getAttribute("name")+"'";
			}
	   }
	   ...
	   rs = st.executeQuery(sql); 
		...
		//int int_total=0;
		 // try{
		 //   String sql2 = "select count(*) from noticeboard union all select count(*) from QNABOARD WHERE noticeboard.writerB = '"+(String)session.getAttribute("name")+"' and QNABOARD.writerQ = '"+(String)session.getAttribute("name")+"'";
